    ...
    public static List<String> performLookup(String command, Statement statement, Location l, CommandSender commandSender, int page, int limit) {
        ...
        try {
            ...
            while (results.next()) {
                ...
                if (!found) {
                    result.add(new StringBuilder(Color.WHITE + "----- " + Color.DARK_AQUA + Phrase.build(Phrase.SIGN_HEADER) + Color.WHITE + " ----- " + ChatUtils.getCoordinates(commandSender, command, worldId, x, y, z, false, false) + "").toString());
                }
                found = true;
